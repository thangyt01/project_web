...
const { fetchGetListTotalOrder, fetchGetOrder, fetchUpdateOrder, fetchDeleteOrder, fetchStatsOrder, fetchCreateOrder, fetchGetUserOrder } = require('./orderService')
...
async function getOrder(req, res) {
    try {
        const results = await fetchGetOrder(req);
        if(results.success) res.end(respondItemSuccess(results.data, results.message))
        else res.end(respondWithError(results.code, results.message, {}))
    } catch (error) {
        res.end(respondWithError(ERROR_CODE_SYSTEM_ERROR, error.message, error))
...
async function getUserOrder(req, res) {
    try {
        const results = await fetchGetUserOrder(req);
        if(results.success) res.end(respondItemSuccess(results.data, results.message))
        else res.end(respondWithError(results.code, results.message, {}))
    } catch (error) {
        res.end(respondWithError(ERROR_CODE_SYSTEM_ERROR, error.message, error))
